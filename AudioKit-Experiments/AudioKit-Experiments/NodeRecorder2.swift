...
open class NodeRecorder2: NSObject {
    // MARK: - Properties
    ...
    public var internalAudioFile: AVAudioFile?
    ...
    public static var currentFileURL = URL(string: "")
    /// Callback type
    ...
    deinit {
        if shouldCleanupRecordings { NodeRecorder2.removeRecordedFiles() }
    }
    ...
    public static func createAudioFile(fileDirectoryURL: URL = URL(fileURLWithPath: NSTemporaryDirectory())) -> AVAudioFile? {
        let filename = "sound1.caf"//createDateFileName() + ".wav"
        ...
        print(settings[AVLinearPCMIsFloatKey] ?? "")
...
//        let audioFile = try? AVAudioFile(forWriting: url, settings: settings, commonFormat: .pcmFormatFloat32, interleaved: false)
    ...
    public static func removeRecordedFiles() {
        for url in NodeRecorder2.recordedFiles {
            try? FileManager.default.removeItem(at: url)
            Log("𝗫 Deleted tmp file at", url)
        }
        NodeRecorder2.recordedFiles.removeAll()
    }
    ...
    public func record() throws {
        ...
        if let path = internalAudioFile?.url.path, !FileManager.default.fileExists(atPath: path) {
            ...
            if let audioFile = NodeRecorder2.createAudioFile(fileDirectoryURL: fileDirectoryURL) {
...
//        }
    ...
    public func createNewFile() {
        ...
        internalAudioFile = NodeRecorder2.createAudioFile(fileDirectoryURL: fileDirectoryURL)
    }
}
